...
internals.properties.prototype.parseProperty = function(name, joiObj, parent, parameterType, useDefinitions, isAlt) {
  ...
  if (property.type === 'object') {
    if (Utilities.hasJoiChildren(joiObj)) {
      ...
      if (useDefinitions === true) {
        property = this.parsePatternObject(property, joiObj.$_terms.patterns[0], name, parameterType, useDefinitions, isAlt);
        let refName = this.definitions.append(name, property, this.getDefinitionCollection(isAlt), this.settings);
        property = {
          $ref: this.getDefinitionRef(isAlt) + refName
        };
 ...
 * @return {Object}
 */
internals.properties.prototype.parsePatternObject = function(property, joiObj, name, parameterType, useDefinitions, isAlt) {
  property.properties = {};

  joiObj.schema._rules.forEach((obj) => {
    let keyName = obj.id;
    let itemName = obj.id;
    let joiChildObj = obj.schema;
